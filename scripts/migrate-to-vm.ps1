...
Write-Host "║  NanoClaw: AKS → VM Migration Script (PowerShell)             ║" -ForegroundColor Cyan
Write-Host "╚════════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "  VM SKU: $vmSku (approx £5/month)"
Write-Host "  Disk: 20GB"
Write-Host "  OS: Ubuntu 22.04 LTS"
Write-Host ""

# === DELETION ===
Write-Host "⚠️  DELETION PHASE" -ForegroundColor Red
...
if ($deleted) {
    ...
    Write-Host "⏱️  Deletion still in progress (running in background). Continuing..."
}
Write-Host ""
...
Write-Host "✅ Network security group created (SSH enabled)"
...
Write-Host "✅ Managed identity assigned"
...
if ($sshReady) {
    Write-Host "✅ VM is SSH-ready"
} else {
    Write-Host "⚠️  VM may still be starting up. Check manually with ssh azureuser@$publicIp"
}
...
Write-Host "  SKU: $vmSku (approx £5/month)"
Write-Host "  Disk: 20GB"
...
Write-Host "   az keyvault secret set --vault-name nanoclaw-kv ``"
Write-Host "     --name CLAUDE-CODE-OAUTH-TOKEN --value ""<token>"""
Write-Host "   az keyvault secret set --vault-name nanoclaw-kv ``"
Write-Host "     --name ANTHROPIC-API-KEY --value ""sk-ant-..."""
Write-Host ""
